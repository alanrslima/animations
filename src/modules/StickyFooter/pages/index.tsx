...
export default () => {
  ...
  const topEdge =
    bottomActions?.y - height + bottomActions?.height + HEADER_HEIGHT;
  const inputRange = [-1, 0, topEdge - 60, topEdge, topEdge + 1];
  const outputRange = [0, 0, 0, 0, -1];
      ...
      <Animated.ScrollView
        ...
        <View>
          ...
          {articleParagraphs.slice(0, 3).map((text, index) => {
            return (
              <View key={index} style={styles.containerFeatured}>
                <Image
      ...
      {bottomActions ? (
        <Animated.View
          style={[
            ...
            styles.animatedButtonActions,
            {
              transform: [
                {translateY: scrollY.interpolate({inputRange, outputRange})},
              ],
            },
          ]}>
